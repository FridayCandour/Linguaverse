class DOM_TTS {
  index = 0;
  nodes: (HTMLElement | undefined)[] = [];
  engine: { speak: (text: string) => void } = {
    speak(text) {
  ...
  constructor(engine?: { speak: (text: string) => void | Promise<void> }) {
    if (engine) {
      this.engine = engine;
    }
  }

  static Map_PUB = (wrap: any[]): any[] => {
    const result: HTMLElement[] = [];
    for (let i = 0; i < wrap.length; i++) {
      const element = wrap[i] as HTMLElement;
  ...
  wrap(WRAP_POINT: HTMLElement) {
    this.nodes = DOM_TTS.Map_CONT(Array.from(WRAP_POINT.children)).filter(
